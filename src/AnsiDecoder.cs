namespace libvt100
{
    ...
    public class AnsiDecoder : EscapeCharacterDecoder, IAnsiDecoder
    {
            ...
            switch ((char)_command)
            {
                ...
                case 'f':
                    {
                        ...
                        {
                            var row = _parameter.Substring(0, separator);
                            var column = _parameter.Substring(separator + 1, _parameter.Length - separator - 1);
                ...
                case 'n':
                    if (_parameter == "6")
                    {
                        ...
                        cursorPosition.X++;
                        cursorPosition.Y++;
                        var row = cursorPosition.Y.ToString();
                        var column = cursorPosition.X.ToString();
        ...
        {
            if ((int)ConsoleKey.F1 <= (int)_key && (int)_key <= (int)ConsoleKey.F12)
            {
                byte[] r = new byte[5];
                r[0] = 0x1B;
                r[1] = (byte)'[';
                int n = (int)_key - (int)ConsoleKey.F1;
                if ((_modifiers & ConsoleModifiers.Shift) != 0)
                    n += 10;
                char tail;
                if (n >= 20)
                    tail = (_modifiers & ConsoleModifiers.Control) != 0 ? '@' : '$';
                else
                    tail = (_modifiers & ConsoleModifiers.Control) != 0 ? '^' : '~';
            ...
            else if (_key == ConsoleKey.LeftArrow || _key == ConsoleKey.RightArrow || _key == ConsoleKey.UpArrow || _key == ConsoleKey.DownArrow)
            {
                byte[] r = new byte[3];
                r[0] = 0x1B;
            ...
            else
            {
                ...
                r[1] = (byte)'[';
                r[3] = (byte)'~';
                if (_key == ConsoleKey.Insert)
                {
                    r[2] = (byte)'1';
                }
                else if (_key == ConsoleKey.Home)
                {
                    r[2] = (byte)'2';
                }
                else if (_key == ConsoleKey.PageUp)
                {
                    r[2] = (byte)'3';
                }
                else if (_key == ConsoleKey.Delete)
                {
                    r[2] = (byte)'4';
                }
                else if (_key == ConsoleKey.End)
                {
                    r[2] = (byte)'5';
                }
                else if (_key == ConsoleKey.PageDown)
                {
                    r[2] = (byte)'6';
                }
                else if (_key == ConsoleKey.Enter)
                {
                    r = new byte[] { 13 };
                }
